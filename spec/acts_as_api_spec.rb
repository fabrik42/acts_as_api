...
describe "acts_as_api" do
  ...
  describe "acts_as_api is enabled" do
    ...
    describe "calling a method in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_only_full_name => [ :full_name ]
    ...
    describe "renaming an attribute in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_rename_last_name => [ :family_name => :last_name ]
    ...
    describe "renaming the node/key of a method in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_rename_full_name => [ :other_full_name => :full_name ]
    ...
    describe "including an association (which doesn't acts_as_api) in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_include_tasks => [ :tasks ]
    ...
    describe "including an association (which does acts_as_api) in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_include_tasks => [ :tasks ]
        Task.acts_as_api
        Task.api_accessible :v1_include_tasks => [ :heading, :done ]
        @response = @luke.as_api_response(:v1_include_tasks)
      end
    ...
    describe "including a scoped association in the api template" do
      ...
      before(:each) do
        User.api_accessible :v1_include_completed_tasks => [ :completed_tasks => "tasks.completed.all" ]
        ...
        Task.acts_as_api
        Task.api_accessible :v1_include_completed_tasks => [ :heading, :done ]
    ...
    describe "creating a sub node in the api template and putting an attribute in it" do
      ...
      before(:each) do
        User.api_accessible :v1_sub_node => [ :sub_nodes => { :foo => :say_something } ]
    ...
    describe "creating multiple sub nodes in the api template and putting an attribute in it" do
      ...
      before(:each) do
        User.api_accessible :v1_nested_sub_node => [ :sub_nodes => { :foo => { :bar => :last_name } } ]
